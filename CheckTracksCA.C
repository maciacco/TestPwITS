                   ...
                   std::string kinefile = "o2sim_Kine.root")
{
  ...
  for (int iH{0}; iH < 7; ++iH) {
    etaPhi[iH] = new TH2F(Form("etaPhi_L%d", iH), Form("Layer %d;#phi (rad);#eta", iH), 65, 0., 6.5, 30, -1.5, 1.5);
  }
  TH2F* etaPhiTot = new TH2F("etaPhiTot", "nITSCls = 7;#phi (rad);#eta", 65, 0., 6.5, 30, -1.5, 1.5);
  ...
  for (auto& evInfo : info) {
    for (auto& part : evInfo) {
      ...
      for (int i{0}; i < 7; ++i) {
        if (part.track.hasHitOnLayer(i)) {
          etaPhi[i]->Fill(part.track.getPhi(), part.track.getEta());
        }
      }
      if (part.track.getNumberOfClusters() == 7) {
        etaPhiTot->Fill(part.track.getPhi(), part.track.getEta());
      }
